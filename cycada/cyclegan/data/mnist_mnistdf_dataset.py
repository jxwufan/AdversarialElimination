...
class MnistMnistdfDataset(BaseDataset):
    def name(self):
        return 'MnistMnistdfDataset'
	...
	self.mnist = resize(self.mnist);
        ...
        self.mnistdf = np.concatenate((x_adv_train, x_train))
	self.mnistdf = resize(self.mnistdf)
        self.mnistdf_label = np.concatenate((y_train, y_train))
